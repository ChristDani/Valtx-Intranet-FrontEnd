...
const NewsPage = () =>{
    ...
    const getData = async (page: number, items: number, titulo: string) => {
        ...
        const itemsList: any = await newsServices.getList(page, items, titulo, -1, 'asc');
    ...
    const getOneItem = async (id: number) => {
        const onlyOneItem = await newsServices.getOne(id);

        const edittttt = onlyOneItem.data

        edittttt.map((item: any) => (
            setEditId(item.iid_news),
    ...
    const confirmOp = async (e: any) => {
        ...
        if (modalState.create) {
            if (editImage != null) {
                const res = await newsServices.create(editImage, editTitle, editDesc, editLink, editOrden, editState, editId);
                getData(currentPage, itemsPorPagina, searchTitle)
                closeModal()
            } else {
            ...
            if (editImage != null) {
                const res = await newsServices.update(editTitle, editDesc, editLink, editOrden, editState, editId, editImage)
            } else {
                const res = await newsServices.update(editTitle, editDesc, editLink, editOrden, editState, editId)
            }
            getData(currentPage, itemsPorPagina, searchTitle)
            closeModal()
        } else if (modalState.delete) {
            const res = await newsServices.delete(editId);
            getData(currentPage, itemsPorPagina, searchTitle)
            closeModal()
        } else {
            alert('detalles')
    ...
    return(
        ...
        <div className="mt-2 pt-4 ml-8 pb-8">
            <h1 className="uppercase font-bold">Mantenedor de Noticias</h1>
                        ...
                        {/* Replace the following <tr> elements with your actual product data */}
                        {
                            datInfo.IsSuccess ? (
                                dataList.map((item: any) => (
                                    <tr className="bg-white border-b hover:bg-gray-50" key={item.iid_news}>
                                        ...
                                        <td className="flex gap-3 items-center justify-center my-auto px-6 h-44">
                                            <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => itemDetails(e, item.iid_news)}>
                                                <svg className="w-6 h-6 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 11h2v5m-2 0h4m-2.592-8.5h.01M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                                </svg>

                                            </Link>
                                            <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => editItem(e, item.iid_news)}>
                                                <svg className="w-6 h-6 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.304 4.844 2.852 2.852M7 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-4.5m2.409-9.91a2.017 2.017 0 0 1 0 2.853l-6.844 6.844L8 14l.713-3.565 6.844-6.844a2.015 2.015 0 0 1 2.852 0Z" />
                                                </svg>
                                            </Link>
                                            <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => deleteItem(e, item.iid_news)}>
                                                <svg className="w-6 h-6 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
