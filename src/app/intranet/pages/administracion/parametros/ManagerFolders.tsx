...
const ManagerFolder = ({close, idCabecera, crear, editar, eliminar} : {
    ...
    const [editNameCabecera, setEditNameCabecera] = useState('Iso 9001');
    //Tipos repositorios
    ...
    const confirmOp = async (e: any) => {
        ...
        if (state === 'create') {
            if (editTitle != null) {
              //const res = await repositorioServices.create(editDoc,editTitle,editCabecera,editState,editId,editidCabecera,editNameRepo,editNameCabecera);
            } else {
                alert('Debe llenar todos los campos')
            }
        } else if (state === 'update') {
            if (editTitle != null) {
                //const res = await repositorioServices.update(editTitle,editCabecera,editState,editId,editidCabecera,editNameRepo,editNameCabecera,editDoc)
            } else {
                //const res = await repositorioServices.update(editTitle,editCabecera,editState,editId,editidCabecera,editNameRepo,editNameCabecera)
            }
                                    ...
                                    dataList.map((item:repositorio)=>(
                                            <tr className="bg-white border-b hover:bg-gray-50" key={item.iid_tabla_detalle}>
                                                ...
                                                <td className="flex gap-1 items-center justify-center my-auto px-4 h-14">
                                                    {editar && <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) =>editItem(e,item.iid_tabla_detalle)}>
                                                    ...
                                                    {eliminar && <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => deleteItem(e, item.iid_tabla_detalle)}>
                                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
