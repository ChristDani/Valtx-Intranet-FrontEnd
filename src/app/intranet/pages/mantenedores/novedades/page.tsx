...
const NovedadesPage = () => {
  ...
  const getOptionsUser = async (id: any, path : string) => {
    ...
    const getData = async (page: number, items: number, titulo: string) => {
        ...
        const itemsList: any = await novedadesServices.getList(page, items, titulo, -1, 'asc');
    ...
    const getOneItem = async (id: number) => {
        const onlyOneItem = await novedadesServices.getOne(id);

        const edittttt = onlyOneItem.data

        edittttt.map((item: any) => (
            setEditId(item.iid_novedad),
    ...
    const confirmOp = async (e: any) => {
        ...
        if (modalState.create) {
            if (Image != null) {
                const res = await novedadesServices.create(Image, editTitle, editDesc, editLink, editOrden, editState, editId);
            ...
            if (Image != null) {
                const res = await novedadesServices.update(editTitle, editDesc, editLink, editOrden, editState, editId, Image)
            } else {
                const res = await novedadesServices.update(editTitle, editDesc, editLink, editOrden, editState, editId)
            }
        } else if (modalState.delete) {
            const res = await novedadesServices.delete(editId);
        } else {
            alert('detalles')
        }
                        ...
                        {/* Replace the following <tr> elements with your actual product data */}
                        {
                            datInfo.IsSuccess ? (
                                dataList.map((item: any) => (
                                    <tr className="bg-white border-b hover:bg-gray-50" key={item.iid_novedad}>
                                        ...
                                        <td className="flex gap-4 items-center justify-center my-auto px-6 h-28">
                                            {optionUser.visualizar && <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => itemDetails(e, item.iid_novedad)}>
                                            ...
                                            {optionUser.editar && <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => editItem(e, item.iid_novedad)}>
                                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            {optionUser.eliminar && <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => deleteItem(e, item.iid_novedad)}>
                                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        modalState.create || modalState.update ? (
                                ...
                                <div className="flex justify-center mb-5 relative gap-1 border border-gray-300 p-1 rounded-xl">
                                    ...
                                    }
                                    {
                                        Image != null ? (
                                            ...
                                            <>
                                                <img className="max-h-44 max-w-[60%] mx-auto relative" src={`/images/novedades/${editImage}`}></img>
                                                <label className="flex absolute px-1 transform translate-y-4 bg-gray-600 bg-opacity-10 backdrop-blur-xl text-center bottom-1 text-black rounded-md max-w-[60%] items-center justify-center">{nameImage}</label>
                                            </>
                                        ) : (
                                            <>
                            ...
                            <>
                                <Link href={editLink} target={redirecction}>
                                    <img className="rounded-lg max-h-72 w-auto mx-auto my-3" src={`/images/novedades/${editImage}`} alt=""></img>
                                </Link>
                                <hr />
