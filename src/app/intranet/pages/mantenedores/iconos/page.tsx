...
const IcoPage = () => {
  ...
  const getData = async (page: number, items: number, titulo: string) => {
    ...
    const itemsList: any = await iconServices.getList(
      page,
      items,
      titulo,
      -1,
  ...
  const validarNombre = (nombre: string, List: any,id:string) => {
    if(modalState.update) {
      return List.some((element:any)=>element.iid_icono !== id && element.vtitulo == nombre);
    }
    return false;
  }
    ...
    if (modalState.create) {
      try {
        if (tipoIcono === "") {
          ...
          const res = await iconServices.create(
            Image,
            editTitle,
            editDesc,
            editLink,
      ...
      if (Image != null) {
        const res = await iconServices.update(
          editTitle,
          editDesc,
          editLink,
          editOrden,
              ...
              <div className="flex justify-start gap-4">
                ...
                <div className="mb-5 relative">
                  ...
                  <select
                    ...
                    value={tipoIcono}
                    className="bg-gray-50 border border-gray-300 rounded-lg p-2"
                    onChange={(e) => setTipoIcono(e.target.value)}
                  >
                    <option hidden key="0" value="0">
                      Seleccione
                    </option>
