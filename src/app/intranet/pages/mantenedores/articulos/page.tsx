...
import { ActionInf, ActionEdit, ActionDel } from "@/app/intranet/componentes/mantenedores/actions";
...
const ArticPage = () => {
  ...
  const getOptionsUser = async (id: any, path : string) => {
    ...
    const getData = async (page: number, items: number, titulo: string) => {
        ...
        const itemsList: any = await articulosServices.getList(page, items, titulo, -1, 'asc');
    ...
    const getOneItem = async (id: number) => {
        const onlyOneItem = await articulosServices.getOne(id);

        const edittttt = onlyOneItem.data

        edittttt.map((item: any) => (
            setEditId(item.iid_articulo),
    ...
    const validarNombre = (nombre: string, List: any,id:string) => {
        if(modalState.update) {
          return List.some((element:any)=>element.iid_articulo !== id && element.vtitulo == nombre);
        }
        return false;
      }
    const confirmOp = async (e: any) => {
        ...
        if (modalState.create) {
            ...
            if (Image != null) {
                const res = await articulosServices.update(editTitle, editDesc, editLink, editOrden, editState, editId, Image)
            } else {
                const res = await articulosServices.update(editTitle, editDesc, editLink, editOrden, editState, editId)
            }
                        ...
                        {/* Replace the following <tr> elements with your actual product data */}
                        {
                            datInfo.IsSuccess ? (
                                dataList.map((item: any) => (
                                    <tr className="bg-white border-b hover:bg-gray-50" key={item.iid_articulo}>
                                        ...
                                        <td className="flex gap-4 items-center justify-center my-auto px-6 h-28">
                                            ...
                                            <div className="flex gap-2 max-lg:hidden">
                                                ...
                                                <ActionDel
                                                    ...
                                                    accion={(e) => deleteItem(e,item.iid_articulo)}
                                                    />
                                                </div>
                                        </td>
                                    </tr>
                        ...
                        modalState.create || modalState.update ? (
                            <form onSubmit={confirmOp} className="mt-5">
                                ...
                                <div className="mb-5 hidden relative">
                                    ...
                                    <input type="file" ref={imageRef}  accept=".jpg, .jpeg, .png" name="vimagen" className="file:hidden bg-gray-50 border border-gray-300 rounded-lg p-2 w-full cursor-pointer" onChange={cambiarImagen}></input>
                                </div>
                                {
                                    errorModal &&
                                ...
                                <div className="flex justify-center relative gap-1 border border-gray-300 p-1 rounded-xl">
                                    ...
                                    }
                                    {
                                        Image != null ? (
                                            ...
                                            <>
                                                <ImagenFront className="max-h-44 max-w-[60%] mx-auto relative" src={`articulos/${editImage}`} alt=""/>
                                                <label className="hidden absolute px-1 transform translate-y-4 bg-gray-600 bg-opacity-10 backdrop-blur-xl text-center bottom-1 text-black rounded-md max-w-[60%] items-center justify-center">{nameImage}</label>
                                            </>
                                        ) : (
                                            <>
                                ...
                                <div className="flex justify-start gap-4">
                                    <div className="mb-5 relative">
                                        ...
                                        <select id="stateItem" className="bg-gray-50 border border-gray-300 rounded-lg p-2" value={editState} onChange={(e) => setEditState(e.target.value)}>
                            ...
                            <>
                                <Link href={editLink} target={redirecction}>
                                    <ImagenFront className="rounded-lg max-h-72 w-auto mx-auto my-3" src={`articulos/${editImage}`} alt=""/>
                                </Link>
                                <hr />
