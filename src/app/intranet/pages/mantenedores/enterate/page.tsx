...
const EnteratePage = () => {
  ...
  const getOptionsUser = async (id: any, path : string) => {
    ...
    const getData = async (page: number, items: number, titulo: string) => {
        ...
        const itemsList: any = await enterateServices.getList(page, items, titulo, -1, 'asc');
                                ...
                                dataList.map((item: any, key) => (
                                    <tr className="bg-white border-b hover:bg-gray-50" key={item.iid_enterate}>
                                        ...
                                        <td className="flex gap-4 items-center justify-center my-auto px-6 h-28">
                                            {optionUser.visualizar && <Link href="" className="font-medium text-blue-600 hover:underline" onClick={(e) => itemDetails(e, item.iid_enterate)}>
