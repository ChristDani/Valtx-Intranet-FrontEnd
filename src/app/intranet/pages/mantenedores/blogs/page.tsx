...
const BlogPage = () => {
  ...
  const getData = async (page: number, items: number, titulo: string) => {
    ...
    const itemsList: any = await blogServices.getList(
      page,
      items,
      titulo,
      -1,
  ...
  const validarNombre = (nombre: string, List: any, id: string) => {
    if (modalState.update) {
      return List.some((element: any) => element.iid_blog !== id && element.vtitulo == nombre);
    }
    return false;
  }
  const confirmOp = async (e: any) => {
    ...
    if (modalState.create) {
      ...
      if (Image != null) {
        const res = await blogServices.update(
          editTitle,
          editDesc,
          editLink,
          editOrden,
                  ...
                  <td className="flex gap-4 items-center justify-center my-auto px-6 h-28">
                    ...
                    <div className="flex gap-2 max-lg:hidden">
                      <ActionInf
                        ...
                        accion={(e) => itemDetails(e, item.iid_blog)}
                      />
                      <ActionEdit
                        permiso={optionUser.editar}
                        accion={(e) => editItem(e, item.iid_blog)}
                      />
                      <ActionDel
                        permiso={optionUser.eliminar}
                        accion={(e) => deleteItem(e, item.iid_blog)}
                      />
                    </div>
                  </td>
                </tr>
              ...
              <div className="mb-5 flex">
                <div className="flex-auto w-28 relative">
                  ...
                  <input
                    type="text"
                    name="iorden"
                    className="bg-gray-50 border border-gray-300 rounded-lg w-3/4 block p-2"
                    value={editOrden}
                    onInput={(e: any) => validarOrder(e.target)}
                  ></input>
                </div>
